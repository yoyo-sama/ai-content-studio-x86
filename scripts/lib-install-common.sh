...
# Models downloaded by the OLD layout live inside the repo ($REPO_ROOT/comfyui/models), where
# the current ComfyUI does not read them. Moving them is instant (same filesystem) and saves
# re-downloading ~150 GB. Merged file by file: the destination may already hold some of them,
# and 'mv dir/*' would choke on a sub-folder that exists on both sides. 'mv -n' never
...
migrate_legacy_models() {
  local legacy="$REPO_ROOT/comfyui/models" f rel moved=0 failed=0
  local files=()
  [ -d "$legacy" ] || return 0
    ...
    if [ -z "$COMFY_CONTAINER" ]; then
      ...
      if [ "$CONFIGFILE" = "$REPO_ROOT/docker-compose.yml" ]; then
        ...
        if [ -d "$REPO_ROOT/comfyui/models" ]; then
          echo "  Legacy model folder detected: $REPO_ROOT/comfyui/models"
          echo "  Its files are moved to the new stack at step 3/5, not downloaded again."
        fi
        create_comfy_stack
